...
from async_service import WorkerConfig, Processor, InputMessage
...
class AsyncFunctionDeployment:
    ...
    def __init__(
        ...
        if INTERNAL_FUNCTION_NAME in functions or FUNCTION_SCHEMA_ENDPOINT.lstrip('/') in functions or RESULT_ENDPOINT.split('/')[1] in functions:
            raise ValueError(
    ...
    def build_async_server_app(self) -> FastAPI:
        ...
        if len(func_names_list) != len(set(func_names_list)):
            raise ValueError("Keys of functions dictionary (converted to lower case) must be unique.")
